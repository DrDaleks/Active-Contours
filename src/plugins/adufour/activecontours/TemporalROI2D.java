...
/**
 * A temporal sequence of {@link ROI2D} objects.
 * 
 * @author Alexandre Dufour
 */
class TemporalROI2D extends ROI4D implements ROIListener, OverlayListener, Iterable<ROI2D>
{
    public static final String              PROPERTY_USECHILDCOLOR = "useChildColor";
    
    protected final TreeMap<Integer, ROI2D> roiSequence            = new TreeMap<Integer, ROI2D>();
    
    protected final Class<? extends ROI2D>  roiClass;
     ...
     */
    public TemporalROI2D(Class<? extends ROI2D> roiClass)
    {
        super();
    ...
    {
        return new ROI2DSequencePainter();
    }
    
    /**
     * Create a new empty 2D ROI slice.
     */
    protected ROI2D createSlice()
    {
        try
        {
    ...
    public void setColor(Color value)
    {
        ...
        try
        {
            ...
            if (!getUseChildColor())
            {
                ...
                try
                {
                    for (ROI2D slice : roiSequence.values())
     ...
     * Returns the ROI slice at given T position.
     */
    public ROI2D getSlice(int t, boolean createIfNull)
    {
        ROI2D result = getSlice(t);
     ...
     */
    public void setTimePoint(int t, ROI2D roi2d)
    {
        ...
        roiSequence.put(Integer.valueOf(t), roi2d);
     ...
     */
    public ROI2D removeSlice(int z)
    {
        // remove the current slice (if any)
        final ROI2D result = roiSequence.remove(Integer.valueOf(z));
    ...
    public Rectangle4D computeBounds4D()
    {
        ...
        for (ROI2D slice : roiSequence.values())
        {
            ...
            if (!bnd2d.isEmpty())
            {
                if (xyBounds == null) xyBounds = (Rectangle2D) bnd2d.clone();
                else xyBounds.add(bnd2d);
            }
        }
        
        // create empty 2D bounds
        if (xyBounds == null) xyBounds = new Rectangle2D.Double();
        ...
        for (ROI2D roi : this)
        {
            int currentZ = roi.getZ();
            if (currentZ < minZ) minZ = currentZ;
            if (currentZ > maxZ) maxZ = currentZ;
        }
        
        return new Rectangle4D.Double(xyBounds.getX(), xyBounds.getY(), minZ, minT, xyBounds.getWidth(), xyBounds.getHeight(), maxZ - minZ + 1, sizeT);
    }
    
    @Override
    public boolean contains(double x, double y, double z, double t)
    {
        final ROI2D roi2d = getSlice((int) t);
        
        if (roi2d != null) return roi2d.getZ() == z && roi2d.contains(x, y);
    ...
    public boolean contains(double x, double y, double z, double t, double sizeX, double sizeY, double sizeZ, double sizeT)
    {
        ...
        {
            final ROI2D roi2d = getSlice(localT);
            if (roi2d == null || roi2d.getZ() < z || roi2d.getZ() >= (z + sizeZ) || !roi2d.contains(x, y, sizeX, sizeY)) return false;
        }
    ...
    public boolean intersects(double x, double y, double z, double t, double sizeX, double sizeY, double sizeZ, double sizeT)
    {
        ...
        {
            final ROI2D roi2d = getSlice(localT);
            if (roi2d != null && roi2d.getZ() >= z && roi2d.getZ() < (z + sizeZ) && roi2d.intersects(x, y, sizeX, sizeY)) return true;
        }
    ...
    public double computeNumberOfContourPoints()
    {
        ...
        {
            final Entry<Integer, ROI2D> firstEntry = roiSequence.firstEntry();
            final Entry<Integer, ROI2D> lastEntry = roiSequence.lastEntry();
            ...
            for (ROI2D slice : roiSequence.subMap(firstKey, false, lastKey, false).values())
    ...
    @Override
    public Iterator<ROI2D> iterator()
    {
    ...
    public class ROI2DSequencePainter extends ROIPainter
    {
        ROI2D getSliceForCanvas(IcyCanvas canvas)
        {
        ...
        public void paint(Graphics2D g, Sequence sequence, IcyCanvas canvas)
        {
            if (isActiveFor(canvas))
            {
                ...
                else if (canvas instanceof IcyCanvas2D)
                {
                    ...
                    final ROI2D slice = getSliceForCanvas(canvas);
